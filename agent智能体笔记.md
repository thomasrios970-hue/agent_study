...
在相应的项目conda独立环境下安装openai和python-dotenv
...
***注：我这里使用了在系统环境变量中读取API的方法,具体在LLM调用函数有体现

> 指定了大语言模型的连接参数，包括API密钥，要调用的具体模型名称，API服务的基地址

### 1.3 封装基础LLM调用函数

定义一个专属的LLM客户端类，把和大模型交互的细节（如API调用、密钥管理、请求格式等）封装起来
...
根据上面的LLM客户端，构建经典智能体范式ReAct，其核心思想为模仿人类解决问题的方式，将推理与行动显示地结合起来，形成一个“思考-行动-观察”的循环
